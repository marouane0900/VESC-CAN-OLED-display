...
void vesc_set_current(float current, int adrCAN) {
...
void vesc_set_current_brake(float current, int adrCAN) {
  uint32_t set_value = current * 1000;
...
int poleprov, minbatprov, maxbatprov, brakeampsprov, maxampsprov, mincalprov, maxcalprov, diamroueprov, accelCANprov, ratioprov, adCAN1prov, adCAN2prov, spCANprov, TCprov, extcomodoprov, on3, on4, dl, val;
...
void setup() {
  ...
  if(dejprog != 1){
    ...
    EEPROM.put(130, adCAN1put);
    EEPROM.put(135, adCAN2put);
    EEPROM.put(140, spCANput);
    EEPROM.put(150, extcomodoput);
    EEPROM.put(150, TCput);
  ...
  EEPROM.get(130, adCAN1);
  EEPROM.get(135, adCAN2);
...
void loop() {
  while(menu == 1){
    spin();
    int kmh = (erpm1 * (diamroue * 0.00254) * 3.14 * 60 * 100) / ((1000 * pole) * ratio);
    ...
    if((accelCAN != 0) && (nopower == 0)){
      if((digitalRead(freining) == 0) || (digitalRead(freinind) == 0)){
        vesc_set_current(powaa, adCAN1);
        vesc_set_current(powaa, adCAN2);
        frein = 0;
      }
      else{
        frein = 1;
        powaa = brakeamps/10;
        vesc_set_current_brake(powaa, adCAN1);
        vesc_set_current_brake(powaa, adCAN2);
        }
    }
    else{
      if(amps < 0){frein = 1;}
  ...
  while((menu == 20) && (choix2 < 8)){
    ...
    u8g2.drawStr(3,8 + (8 * 3),"   Brake rate");
    u8g2.drawStr(3,8 + (8 * 4),"   Accel rate");
  ...
  if(goexit == 1){
    ...
    EEPROM.put(130, adCAN1prov);
    EEPROM.put(135, adCAN2prov);
    EEPROM.put(140, spCANprov);
    EEPROM.put(150, extcomodoprov);
    EEPROM.put(160, TCprov);
    pole = poleprov;
    minbat = minbatprov;
    maxbat = maxbatprov;
